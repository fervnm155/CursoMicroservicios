...
namespace Ordering.Infrastructure.Persistence
{
    public class OrderContext : DbContext
    {
            ...
            foreach(var entry in ChangeTracker.Entries<EntityBase>())
            {
                switch (entry.State)
                {
                    case EntityState.Modified:
                        ...
                        entry.Entity.LastModifiedBy = "User";//pendiente
                        break;
        ...
        private static LambdaExpression MultiTenantExpression<T>(OrderContext context)where T : EntityBase, IMultiTenant
        {
